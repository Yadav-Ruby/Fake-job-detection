"""
connectors/foundit.py
FoundIt (formerly Monster India) connector.
"""
...
class FoundItConnector(BaseConnector):
    ...
    SEARCH_URL = "https://www.foundit.in/srp/results?query={keywords}"

    SELECTORS = {
        "job_title": [".jd-title", "h1.job-title", "h1", ".title"],
        "company": [".company-name a", ".company-name", ".company", "[class*='company']"],
        "location": [".location", ".loc", "[class*='location']"],
        "salary": [".salary", ".ctc", "[class*='salary']"],
        "description": [".job-desc", ".jd-description", ".description", "#jobDescription", "[class*='description']"],
        "posted_date": [".posted-date", ".date", "[class*='posted']"],
        "skills": [".skills a", ".skill-tag", "[class*='skills'] a"],
    }
    ...
    def extract_job_data_bs4(self, html: str, url: str) -> Optional[RawJob]:
        ...
        return RawJob(
            job_title=title or "FoundIt Role",
            company_name=company or "FoundIt Company",
