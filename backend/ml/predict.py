...
def predict_job(job: dict, verbose: bool = False) -> MLPrediction:
    ...
    # Confidence = agreement between models (low std = high agreement)
    scores = [
        prediction.xgboost_score,
        prediction.random_forest_score,
        prediction.isolation_forest_score,
    ]
    score_std = float(np.std(scores))
    prediction.confidence = round(max(0, 100 - score_std), 2)

    # Step 6: Feature importance for this prediction
