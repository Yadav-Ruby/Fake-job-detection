...
class LinkedInConnector(BaseConnector):
    ...
    def extract_job_data_bs4(self, html: str, url: str) -> Optional[RawJob]:
        ...
        body_text = soup.get_text()[:800].lower()
        ...
        if not description or len(description) < 50:
            ...
            skip = ["sign in", "join now", "cookie", "privacy policy", "terms of service", "copyright"]
        ...
        for sel in self.SELECTORS["posted_date"]:
            el = soup.select_one(sel)
            if el:
                posted_date = el.get_text(strip=True)
                break
        ...
        for el in soup.select(self.SELECTORS["criteria_items"]):
            txt = el.get_text(strip=True)
            txt_lower = txt.lower()
            if "employee" in txt_lower or "people" in txt_lower:
                company_size = txt
            elif "seniority" in txt_lower or "level" in txt_lower:
                seniority = txt
            elif "employment" in txt_lower or "type" in txt_lower:
                employment = txt
            elif "industries" in txt_lower or "industry" in txt_lower:
                industry = txt
                
        recruiter_name = ""
        recruiter_title = ""
        rec_el = soup.select_one(self.SELECTORS["recruiter"])
        if rec_el:
            lines = [l.strip() for l in rec_el.get_text(separator="\n").splitlines() if l.strip()]
            recruiter_name = lines[0] if len(lines) > 0 else ""
            recruiter_title = lines[1] if len(lines) > 1 else ""
            
        mode = "Unknown"
        workplace_el = soup.select_one(self.SELECTORS["workplace_badge"])
        if workplace_el:
            badge_txt = workplace_el.get_text().lower().strip()
            if "remote" in badge_txt: mode = "Remote"
            elif "hybrid" in badge_txt: mode = "Hybrid"
            elif "on-site" in badge_txt or "onsite" in badge_txt: mode = "On-site"
            
        if mode == "Unknown":
            text_all = (location + " " + description).lower()
            if "remote" in text_all: mode = "Remote"
            elif "hybrid" in text_all: mode = "Hybrid"
            elif any(s in text_all for s in ["on-site", "onsite", "in-office", "in office"]):
                mode = "On-site"
