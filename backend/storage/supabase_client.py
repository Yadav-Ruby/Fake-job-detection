...
Usage:
    from backend.storage.supabase_client import get_client, insert_job
"""
...
def insert_model_training_data(data: dict) -> bool:
    """
    Insert a record into model_training_data table if not already exists.
    """
    sb = get_client()
    try:
        desc = data.get("job_description")
        if desc:
            existing = sb.table("model_training_data").select("id").eq("job_description", desc).limit(1).execute()
            if existing.data:
                return False
        
        result = sb.table("model_training_data").insert(data).execute()
        return bool(result.data)
    except Exception as e:
        logger.warning(f"insert_model_training_data failed: {e}")
        return False
...
# Add these functions to your existing supabase_client.py
...
def get_user_report_count(
    ...
    """
    Get user report counts for a job URL and/or company name.

    Returns:
        {"verified": int, "pending": int, "total": int}
    """
    sb = get_client()
    counts = {"verified": 0, "pending": 0, "total": 0}
    ...
    try:
        ...
        logger.warning(f"get_user_report_count failed: {e}")

    return counts


def insert_user_report(report_data: dict) -> bool:
    """
    Insert a user scam report.

    Expected keys:
        job_url, job_description, company_name,
        contact_method, experience, contact, user_id (optional)
    """
    sb = get_client()
    try:
        report_data.setdefault("status", "pending")
        result = sb.table("user_reports").insert(report_data).execute()
        return bool(result.data)
    except Exception as e:
        logger.warning(f"insert_user_report failed: {e}")
        return False


def get_keyword_dictionary() -> list[dict]:
    """Fetch all keywords from keyword_dictionary table."""
    sb = get_client()
    try:
        result = sb.table("keyword_dictionary").select("*").execute()
        return result.data or []
    except Exception as e:
        logger.warning(f"get_keyword_dictionary failed: {e}")
        return []


def upsert_keyword(keyword: str, weight: float, category: str) -> bool:
    """Add or update a keyword in the dictionary."""
    sb = get_client()
    try:
        result = (
            sb.table("keyword_dictionary")
            .upsert(
                {"keyword": keyword, "weight": weight, "category": category},
                on_conflict="keyword",
            )
            .execute()
        )
        ...
        logger.warning(f"upsert_keyword failed: {e}")
        return False