...
def extract_text_stats(text: str) -> dict:
    ...
    capital_letter_ratio = sum(1 for char in text if char.isupper()) / len(text) if text else 0.0
...
def batch_extract_ner(texts: list) -> list:
    results = [None] * len(texts)
    truncated = [t[:500] if t else "" for t in texts]

    try:
        with nlp.select_pipes(enable=["tok2vec", "ner"]):
            docs = list(nlp.pipe(truncated, batch_size=NER_BATCH_SIZE))
    except Exception:
        try:
            docs = list(nlp.pipe(truncated, batch_size=NER_BATCH_SIZE))
        except Exception:
            return [_empty_ner() for _ in texts]
...
def extract_fraud_category_scores(desc, payment, contact, urgency, email, domain, ner) -> dict:
    ...
    id_risk = 0.0
    if not email["has_corporate_email"]: id_risk += 30
    if email["has_disposable_email"]: id_risk += 50
    if email["has_personal_email"]: id_risk += 20
    if domain["domain_reputation_score"] < 40: id_risk += 30
    if not ner["has_company_name"]: id_risk += 25
    identity_risk_score = min(100.0, id_risk)

    rec_risk = 0.0
    if "no interview" in desc_lower or "without interview" in desc_lower: rec_risk += 40
    if "no experience" in desc_lower or "freshers can apply" in desc_lower: rec_risk += 30
    if "no resume" in desc_lower: rec_risk += 30
    if "no background check" in desc_lower: rec_risk += 20
...
def extract_numeric_features(job: dict, ner: dict = None) -> dict:
    features = {}
    ...
    features.update(extract_readability_features(desc))
    features.update(extract_text_stats(desc))
    ...
    features["has_skills"] = 1 if skills else 0
...
def build_feature_dataframe(jobs: list, fit_tfidf: bool = True) -> pd.DataFrame:
    ...
    if VERBOSE:
        print(f"\nExtracting features from {total} jobs...")

    print(f"   Running NER on {total} descriptions in batches of {NER_BATCH_SIZE}...")
    all_texts = [job.get("job_description", "") or "" for job in jobs]
    all_ner_results = []

    for i in range(0, total, NER_BATCH_SIZE):
        batch_texts = all_texts[i: i + NER_BATCH_SIZE]
        batch_ner = batch_extract_ner(batch_texts)
        all_ner_results.extend(batch_ner)
        print(f"   NER: {min(i + NER_BATCH_SIZE, total)}/{total} done...", end="\r")
    print()

    print(f"   Extracting numeric features in batches of {FEATURE_BATCH_SIZE}...")
    numeric_features = []

    for i in range(0, total, FEATURE_BATCH_SIZE):
        batch_jobs = jobs[i: i + FEATURE_BATCH_SIZE]
        batch_ner = all_ner_results[i: i + FEATURE_BATCH_SIZE]
        for job, ner in zip(batch_jobs, batch_ner):
            numeric_features.append(extract_numeric_features(job, ner=ner))
        print(f"   Numeric: {min(i + FEATURE_BATCH_SIZE, total)}/{total} done...", end="\r")
    print()
    ...
    print(f"   Generating sentence embeddings for {total} descriptions...")
