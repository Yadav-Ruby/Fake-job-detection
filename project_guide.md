...
### Step 3: Run the Application (Choose Mode)

- **Option A: Backend & Scraper Only (Without Frontend)**
  Run the backend API directly:
  ```powershell
  python backend/main.py
  ```
  *(This will start the FastAPI server at http://localhost:8000/ and make the API endpoints and Docs accessible, without mounting the frontend).*

- **Option B: Unified Application (Backend + Frontend)**
  Run the master starter file `app.py`:
  ```powershell
  python app.py
  ```
  *(This starts the FastAPI server and serves the compiled Next.js frontend pages).*

### Step 4: Open in Your Browser
Open your web browser (Chrome, Edge, or Firefox) and go to:
- **Frontend App (Only in Option B)**: [http://localhost:8000/](http://localhost:8000/)
