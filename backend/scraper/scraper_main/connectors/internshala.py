...
class InternshalaConnector(BaseConnector):
    ...
    def extract_job_links_bs4(self, html: str, base_url: str) -> list[str]:
        ...
        for el in soup.find_all("a", href=True):
            href = el["href"]
            if "/internship/detail/" in href:
                links.append(urljoin(base_url, href))
                
        if not links:
            raw = re.findall(r'href=["\'](/internship/detail/[a-zA-Z0-9\-_%]+/?)["\']', html)
            links = [urljoin(base_url, p) for p in raw]
    ...
    def extract_job_data_bs4(self, html: str, url: str) -> Optional[RawJob]:
        ...
        stipend = ""
        for sel in self.SELECTORS["stipend"]:
            el = soup.select_one(sel)
            if el:
                stipend = el.get_text(strip=True)
                break
                
        duration = ""
        for sel in self.SELECTORS["duration"]:
            el = soup.select_one(sel)
            if el:
        ...
        apply_by = ""
        for sel in self.SELECTORS["apply_by"]:
            el = soup.select_one(sel)
            if el:
                apply_by = el.get_text(strip=True)
                break
        ...
        for sel in self.SELECTORS["skills"]:
            els = soup.select(sel)
            if els:
                skills = [e.get_text(strip=True) for e in els if e.get_text(strip=True)]
                break
