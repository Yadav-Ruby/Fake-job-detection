...
export default function AdminDashboardPage() {
  const { analyses, reports, domains } = useStore();

  const totalUsersCount = 184; // mock
  const activeScannersCount = analyses.length;
  const pendingReportsCount = reports.filter(r => r.status === 'pending').length;
  const blacklistedDomainsCount = domains.length;
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-2 text-[10px] font-mono text-slate-400">
              <div className="flex justify-between border-b border-slate-900 pb-1">
                <span className="text-slate-600">00:24:12</span>
                <span className="text-slate-300">WHOIS parsed successfully amazon-jobsite.com</span>
              </div>
              <div className="flex justify-between border-b border-slate-900 pb-1">
                <span className="text-slate-600">00:22:45</span>
                <span className="text-slate-300">Recruiter verify request for hr-support@amazon-jobsite.com</span>
              </div>
              <div className="flex justify-between border-b border-slate-900 pb-1">
                <span className="text-slate-600">00:19:02</span>
                <span className="text-cyber-rose font-bold">CRITICAL threat flagged: check refund scheme detected</span>
              </div>
              <div className="flex justify-between border-b border-slate-900 pb-1">
                <span className="text-slate-600">00:12:15</span>
                <span className="text-slate-300">Admin credentials verified. Token auth success.</span>
              </div>
            </div>
          </div>
