...
class ShineConnector(BaseConnector):
    ...
    def extract_job_data_bs4(self, html: str, url: str) -> Optional[RawJob]:
        ...
        company = ""
        for sel in [s.strip() for s in self.SELECTORS["company"].split(",")]:
            el = soup.select_one(sel)
            if el:
        ...
        edu = ""
        for sel in [s.strip() for s in self.SELECTORS["education"].split(",")]:
            el = soup.select_one(sel)
            if el:
                edu = el.get_text(strip=True)
                break
                
        skills = []
        for sel in [s.strip() for s in self.SELECTORS["skills"].split(",")]:
            els = soup.select(sel)
            if els:
