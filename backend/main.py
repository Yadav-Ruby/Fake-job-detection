...
async def save_user_profile(user: UserProfilePayload):
    try:
        ...
        existing_profile = sb.table("user_profiles").select("*").eq("email", user.email).limit(1).execute()
        if existing_profile.data:
            prof = existing_profile.data[0]
        ...
        try:
            ...
            for u in getattr(auth_users_response, "users", []):
                if u.email == user.email:
                    user_id = u.id
                    break
        except Exception as auth_list_err:
            print(f"Auth list error: {auth_list_err}")
        ...
        if user_id:
            ...
            sb.table("user_profiles").upsert(profile_data, on_conflict="email").execute()
        else:
            # 3. Create new user via Auth Admin (fires trigger automatically)
            user_res = sb.auth.admin.create_user({
                "email": user.email,
                "password": f"DummyPasskey{random.randint(1000, 9999)}!",
                "email_confirm": True,
                "user_metadata": {
                    "full_name": user.name
                }
        ...
        # 4. Fetch and return
        profile_res = sb.table("user_profiles").select("*").eq("id", user_id).limit(1).execute()
        if profile_res.data:
...
async def save_job(payload: SavedJobPayload):
    try:
        ...
        sb.table("saved_jobs").upsert(data, on_conflict="user_email,job_id").execute()
        return {"status": "saved"}
    except Exception as e:
        traceback.print_exc()
        raise HTTPException(status_code=500, detail=str(e))
